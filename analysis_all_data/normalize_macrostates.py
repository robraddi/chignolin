...
outdir = f'{dir}/normalized_{stat_model}_{data_uncertainty}_sigma'
biceps.toolbox.mkdir(outdir)

# NOTE: IMPORTANT: Create multiple trials to average over and check the deviation of results
check_dirs = next(os.walk(f"{dir}/normalized_{stat_model}_{data_uncertainty}_sigma/"))[1]
trial = len(check_dirs)

outdir = f'{dir}/normalized_{stat_model}_{data_uncertainty}_sigma/{nsteps}_steps_{nreplicas}_replicas_{n_lambdas}_lam__swap_every_{swap_every}_change_Nr_every_{change_Nr_every}_trial_{trial}'
...
#print(f"macrostate_populations = {macrostate_populations}")

# Count the number of microstates in each macrostate
microstate_counts = grouped.count()["population"]
total_states = microstate_counts.sum()
total_macrostates = 3.
# equation for scaling factor
scaling_factor = 1 / (total_macrostates*macrostate_populations["population"].to_numpy())

# get new populations for each set of microstates
new_populations = []
for macrostate in range(len(macrostate_populations)):
    indices = np.where(macrostate == df_pops["macrostate"].to_numpy())[0]
    for index in indices:
        new_populations.append((index, df_pops.iloc[[index]]["population"].to_numpy()*scaling_factor[macrostate]))

#print(df_pops)
#print(new_populations)
# reorder the populations using the saved indices
ordered_populations = []
for i in range(len(new_populations)):
    for k in range(len(new_populations)):
        if i == new_populations[k][0]:
            ordered_populations.append(float(new_populations[k][1]))
            break
#print(ordered_populations)
df_pops["population"] = ordered_populations
msm_pops["population"] = ordered_populations

grouped = df_pops.groupby(["macrostate"])
macrostate_populations = grouped.sum().drop("microstate", axis=1)
#print(f"new macrostate_populations = {macrostate_populations}")

energies = -np.log(np.array(ordered_populations))
...
df_pops.columns = ["macrostate"]
df_pops["microstate"] = df_pops.index.to_numpy()
df_pops["population"] = msm_pops[msm_pops.columns[0]].to_numpy()
#df_pops.to_csv(f"{outdir}/{stat_model}_{data_uncertainty}_sigma__reweighted_populations.csv")
grouped = df_pops.groupby(["macrostate"])
#macrostate_populations = grouped.sum().drop("microstate", axis=1)
...
df = pd.read_csv(f"{data_dir}/inverse_distances_k{nstates}_msm_assignments.csv", index_col=0, skiprows=0, comment='#')
#msm_pops = pd.read_csv(f"{data_dir}/inverse_distances_k{nstates}_msm_pops.csv", index_col=0, skiprows=0, comment='#')
_msm_pops = msm_pops[msm_pops.columns[0]]
if all_data or noe_only:
    ...
    noe_prior = np.array([w*noe_model[i] for i,w in enumerate(_msm_pops.to_numpy())]).sum(axis=0)
...
if all_data or J_only:
    ...
    J_prior = np.array([w*J_model[i] for i,w in enumerate(_msm_pops.to_numpy())]).sum(axis=0)
...
if all_data or cs_only:
    ...
    cs_prior = np.array([w*cs_model[i] for i,w in enumerate(_msm_pops.to_numpy())]).sum(axis=0)
...
try:
    ...
    if all_data or noe_only:
        ...
        for k,col in enumerate(columns):
            ...
            sse.append(np.array([(noe_reweighted[i] - noe_Exp[i])**2/(mlp.iloc[-1][col]**2) for i in range(len(noe_Exp))]).sum()/len(noe_Exp))
        print(sse)
        _sse.append(sse)
    ...
    if all_data or J_only:
        ...
        sse = []
        for k,col in enumerate(columns):
            sse.append(np.array([(J_reweighted[i] - J_Exp[i])**2/(mlp.iloc[-1][col]**2) for i in range(len(J_Exp))]).sum()/len(J_Exp))
        print(sse)
        _sse.append(sse)
    ...
    if all_data or cs_only:
        ...
        sse = []
        for k,col in enumerate(columns):
            sse.append(np.array([(cs_reweighted[i] - cs_Exp[i])**2/(mlp.iloc[-1][col]**2) for i in range(len(cs_Exp))]).sum()/len(cs_Exp))
        _sse.append(sse)

except(Exception) as e:
    print(e)

sse = 0.5*np.array(_sse).sum()
print(sse)
