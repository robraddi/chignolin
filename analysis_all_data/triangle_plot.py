    ...
    for i in range(len(_labels)):
        ...
        if i == 0:
            ...
            for j in range(len(list(stat_models.keys()))):
                ...
                ax.text(x, y,
                        '%s'%(list(stat_models.keys())[j]),
                        transform=ax.transAxes, fontsize=16, color="b", #stat_model_color_dict[labels[l]],
                        verticalalignment='top',
                        fontweight="bold"
                        )
    ...
    for stat_model in stat_models:
        for FF in FF_list:
            for clusters in _states_:
                files = biceps.toolbox.get_files(f"./{FF}/nclusters_{clusters}/{stat_model}*/*/*__reweighted_populations.csv")
                # Get the files for prior pops and assignments (microstate --> macrostate)
                # ..for each FF/microstate clustering
                data_dir = f"{sys_name}/{FF}/nclusters_{clusters}"
                assignment_files = biceps.toolbox.get_files(f"{data_dir}/*_msm_assignments.csv")
                prior_pops_files = biceps.toolbox.get_files(f"{data_dir}/*_msm_pops.csv")
