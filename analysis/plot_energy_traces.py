...
#stat_model,nreplicas,data_uncertainty = "BayesianModel", 1, "single"
#stat_model,nreplicas,data_uncertainty = "Good-Bad", 8, "single"
stat_model,nreplicas,data_uncertainty = "StudentsModel",8, "single"
#stat_model,nreplicas,data_uncertainty = "GaussianModel", 8, "multiple"

outdir = f"*/nclusters_{energies}/{stat_model}_{data_uncertainty}_sigma/10000000_steps_{nreplicas}_replicas_2_lam__swap_every_0_change_Nr_every_0_trial_*"
...
gs = gridspec.GridSpec(3, 2, width_ratios=(4, 1), wspace=0.02)
...
ax32 = fig.add_subplot(gs[2,1], sharey=ax3)
axs_ = [ax12,ax22,ax32][::-1]
...
ax3.set_xlim(left=np.min(x), right=np.max(x))
...
ax2.legend([handles[idx] for idx in order],[labels[idx] for idx in order], loc='center left', bbox_to_anchor=(1.30, 0.5), fontsize=label_fontsize)
...
fig.savefig(f"{stat_model}_iZ_distributions.png", dpi=600)
