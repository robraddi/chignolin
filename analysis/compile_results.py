...
if plot_population_bar_charts:
    ...
    # NOTE: TODO: HELP: FIXME: IMPORTANT: ##########################################
                ...
                #if stat_model == "GaussianSP": continue
                #if stat_model == "SingleSigma": continue
                #if stat_model == "StudentsOut": continue
    # NOTE: TODO: HELP: FIXME: IMPORTANT: ##########################################
...
#            df = biceps.toolbox.change_stat_model_name(df, sm="Students", new_sm="Students_old")
#            df = df.iloc[np.where(df["stat_model"] != "Students_old")[0]]
#            students_model = "Students"
#            df = biceps.toolbox.change_stat_model_name(df, sm="StudentsV2", new_sm=students_model)
#            gb_model = "Good-and-Bad"
#            df = biceps.toolbox.change_stat_model_name(df, sm="GB", new_sm=gb_model)
#            outliers_model = r"Students $\{\sigma_{i},\beta_{i}=1\}$"
#            df = biceps.toolbox.change_stat_model_name(df, sm="Outliers", new_sm=outliers_model)
#            stat_models = [gb_model, students_model, "Gaussian", outliers_model]

            #df = df.iloc[np.concatenate([np.where(df["stat_model"] == stat_model)[0] for stat_model in stat_models])]
            ...
            model_labels = ["Exp", "MSM", "Bayesian", "Good-Bad", "Student's", "Gaussian"]
...
#            ax = fig.add_subplot(111)
            ...
            if sys_name == "CLN001": ax.legend(["Folded","Misfolded", "Unfolded"], loc="best", fontsize=16)
...
#stat_models = [gb_model, students_model, "Gaussian", outliers_model]
###############################################################################
#stat_models = [GB_model,"Gaussian","Students","Students-t"] #"Outliers"]
#stat_models = [GB_model,"Gaussian",students_model,"Students-t", "Outliers", "OutliersSP"]
#stat_models = [GB_model,"Gaussian",students_model,"Outliers"]
#stat_models = [GB_model,"Good-Bad",students_model,"StudentsModel"]
#stat_models = [GB_model,"Gaussian",students_model,"test"]
